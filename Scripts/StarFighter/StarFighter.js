...
function StarFighter() {
	...
	var image = $('<img class="StarFighter" width="' + width + 'px" height="'
			+ height + 'px" src="' + URL + 'StarFighter.png" />');
	var firing = false;
	var bullets = {};
	var size = 0;
	var fireType = 0;
	...
	var fire = function() {
		...
		if (fireType == 0) {
			bullets[size] = new Bullet(x + width, Math.floor(y + height / 2),
					speed * 3, 0, 0, damage, side);
			canvas.append(bullets[size++]);
		} else if (fireType == 1) {
			bullets[size] = new Bullet(x + width,
					Math.floor(y + height / 2) + 3, speed * 3, 0, 0, damage,
					side);
			canvas.append(bullets[size++]);
			bullets[size] = new Bullet(x + width,
					Math.floor(y + height / 2) - 3, speed * 3, 0, 0, damage,
			...
			bullets[size] = new Bullet(x + width, Math.floor(y + height / 2),
					speed * 3, -5, 1, damage, side);
			canvas.append(bullets[size++]);
			bullets[size] = new Bullet(x + width,
					Math.floor(y + height / 2) + 3, speed * 3, 0, 1, damage,
					side);
			canvas.append(bullets[size++]);
			bullets[size] = new Bullet(x + width,
					Math.floor(y + height / 2) - 3, speed * 3, 0, 1, damage,
	...
	var keyHandler = function() {
		if (keys[13]) // enter
			fire();
		if (keys[65] || keys[97]) // a || A - left
			x = Math.max(0, x - speed);
		if (keys[87] || keys[119]) // w || W - up
			y = Math.max(0, y - speed);
		if (keys[68] || keys[100]) // d || D - right
			x = Math.min(canvas.getWidth() - width, x + speed);
		if (keys[83] || keys[115]) // s || S - down
	...
	var kill = function() {
		fighter.remove();
		visible = false;
		canvas.append(new Explosion(x + Math.floor(width / 2) - 16, y
				+ Math.floor(height / 2) - 16, function() {
			new TitleScreen();
		}));
	};
	...
	this.getDimensions = function() {
		return {
			...
			'1' : new Triangle(new Point(x + width / 3, y), new Point(
					x + width, y + height / 2), new Point(x + width / 3, y
					+ height))
		};
	};
